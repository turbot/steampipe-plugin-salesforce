...
func SalesforceCampaign(ctx context.Context, p *plugin.Plugin) *plugin.Table {
	tableName := "Campaign"
	...
	return &plugin.Table{
		...
		Columns: mergeTableColumns(ctx, p, cols, []*plugin.Column{
			...
			// Other columns
			{Name: "created_by_id", Type: proto.ColumnType_STRING, Description: "The id of the user who created the  price book, with the date and time of creation."},
